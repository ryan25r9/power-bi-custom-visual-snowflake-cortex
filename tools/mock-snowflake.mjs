...
const EVENTS = [
    ...
    // Since Apr 2026 SQL tool blocks stream as system_execute_sql (was cortex_analyst_text_to_sql).
    ["response.tool_use",
        { content_index: 0, tool_use_id: "toolu_1", type: "system_execute_sql", name: "Analyst1", input: { query: "top regions by sales" } }],
    ["response.tool_result.status",
        { tool_use_id: "toolu_1", tool_type: "system_execute_sql", status: "executing_sql", message: "Executing SQL" }],
    ["response.tool_result",
        { content_index: 0, tool_use_id: "toolu_1", type: "system_execute_sql", name: "Analyst1", status: "success", content: [{ type: "json", json: { query_id: "01a-mock-qid", sql: "SELECT region, SUM(sales) s FROM t GROUP BY 1 ORDER BY 2 DESC", text: "query ran" } }] }],
    ["response.text.delta",
